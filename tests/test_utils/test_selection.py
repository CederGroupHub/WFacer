...
def test_initial_selection(subspace):
    ...
    selected_ids = select_initial_rows(femat,
                                       ...
                                       num_external_terms=num_external_terms,
                                       keep_indices=keep)
    npt.assert_array_equal(selected_ids,
                           np.arange(len(femat)))
    ...
    selected_ids = select_initial_rows(femat,
                                       ...
                                       num_external_terms=num_external_terms,
                                       keep_indices=keep)
    npt.assert_array_equal(selected_ids[: len(keep)],
                           keep)
...
def test_addition(subspace):
    ...
    for iid, ii in enumerate(selected_ids[len(keep):]):
        best_indices = np.array(selected_ids[: len(keep) + iid + 1])
        a_old = np.concatenate((old_femat[:, :len(femat[0]) - num_external_terms],
                                a[selected_ids[: len(keep) + iid]]),
                               axis=0)
        a_best = np.concatenate((old_femat[:, :len(femat[0]) - num_external_terms],
                                 a[best_indices, :]),
                                axis=0)
        ...
        for _ in range(5):
            ...
            a_try = np.concatenate((old_femat[:, :len(femat[0]) - num_external_terms],
                                    a[trial_indices, :]),
                                   axis=0)
