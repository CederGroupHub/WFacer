...
from sparselm.model import OverlapGroupLasso
    ...
    if hasattr(sparselm.model, parent_classname):
        ...
        if hasattr(base, parent_classname):
            ...
            if hasattr(base, parent_classname):
                ...
                raise ValueError(f"sparse-lm does not have parent"
                                 f" class {parent_classname}")
...
for name in all_estimator_names:
    for parent_class in unsupported_parents:
        if issubclass(getattr(getattr(sparselm, "model"),
                              name),
                      parent_class):
            unsupported_estimator_names.append(name)
supported_estimator_names = [name for name in all_estimator_names
                             if name not in unsupported_estimator_names]
...
def estimator_factory(estimator_name, **kwargs):
    ...
    if class_name not in supported_estimator_names:
        raise ValueError(f"Estimator {class_name} is not supported"
                         f" by sparse-lm automation!")
    cls = getattr(getattr(sparselm, "model"), class_name)
    return cls(**kwargs)


def prepare_estimator(cluster_subspace,
                      estimator_name,
                      use_hierarchy=True,
    ...
    # For other cases, groups and hierarchy are always not required.
