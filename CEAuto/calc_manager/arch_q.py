...
class ArchqueueManager(BaseManager, metaclass=ABCMeta):
    """
    ...
    def _submit_single(self, eid):
        """Submit a single computation in archieve.

        It is your responsibility to check that:
        1, The corresponding folder has all required vasp inputs inside.
        2, The corresponding folder is not double-computed for the same
           type of computation.
        """
        ...
        os.chdir(self._root)  # Move back to root dir!


class ArchsgeManager(ArchqueueManager):
    """SGE queue calculation manager class.
    ...
    def entree_in_queue(self, entry_ids):
        """Check ab-initio task status for given entree.
        ...
        for job in all_jobs:
            if (isinstance(job, dict) and
                job['JB_name'].find(self._root_name) == 0):
