...
class MongovaspReader(BaseReader):
    """Calculation reader class.

    To read calculation results from MongoDB. Current implementations
    only support vasp.

    This class only serves as accessor to the data warehouse, and will
    not change the fact table.
    """

    DEFAULT_MONGO_PATH = os.path.join(atomate.__path__[0],
                                      'config/db.json')
    ...
    def __init__(self, md_file=None, **kwargs):
        ...
        Args:
            md_file(str):
                Path to mongodb setting file. The calculations will be read
                from this database.
        """
    ...
    def check_convergence_status(self,entry_ids):
        ...
        Args:
            entry_ids(List of ints):
                ...
                Must be provided.
        Returns:
            List[bool]: each shows whether the calculation
        ...
        for eid in entry_ids:
            ...
            r_conv = (rd.get('calcs_reversed', [{}])[0]
                      .get('has_vasp_completed', False))
            s_conv = (sd.get('calcs_reversed', [{}])[0]
                      .get('has_vasp_completed', False))

            # Both static and relaxation convergence.
    ...
    def load_structures(self, entry_ids):
        ...
        Returns:
            list[pymatgen.Structure]: all composed of 
        ...
        for eid in entry_ids:
            ...
            if str_d is None:
                raise ValueError("Specified entry {} does not have structure!"
                                 .format(eid))
            else:
    ...
    def get_single_calc(self,eid):
        ...
        Returns:
            Dict, Dict:
                Relaxation calculation data in dictionary,
                and static calculation data in dictionary.
        """
        entry_name = 'ce_{}_{}'.format(self.root_name, eid)
        relax_name = entry_name + '_optimization'
        static_name = entry_name + '_static'

        relax_calcd = (self._mongod.collection
                       .find_one({'task_label': relax_name}) or {})
        static_calcd = (self._mongod.collection
                        .find_one({'task_label':static_name}) or {})

        if len(relax_calcd) == 0 and len(static_calcd) == 0:
            raise ValueError("Specified entry {} not found!"
                             .format(eid))

        return relax_calcd, static_calcd

    def _load_property_by_name(self, entry_ids, name='energy'):
        """Load a single type of property of structures.
        ...
        for eid in entry_ids:
            ...
            if name == 'energy':
                ...
                if e is None:
                    raise ValueError("Specified entry {} missing energy!"
                                     .format(eid))
